...
1. `NEW`: issue reported (initial state).  Issues in this state are in the
   ...
   2. `Node.js Team`: Issues assigned by `H1 Triage`, or issues that Node.js
      ...
      because its disposition is obvious, or some other reason we decide not to
...
2. `TRIAGED`: State for an issue when it is a non-duplicate and resolvable.
   ...
   problem. What the resolution should be should be described in a comment on
   the issue when it is moved to `TRIAGED`.
   - ...: There is no explicit states for an issue as it is in process of
     getting resolved. Comments on the issue when someone is working on it are
     useful, as is assignment of the issue to a person who agress to be
     responsible for making sure it is resolved.
...
7. Disclosure: This not a HackerOne report state, but it is the final state
   from our perspective. All issues should be disclosed, eventually, for the
   information they give to the Node.js community about using Node.js security,
   and to contribute to the transparency and credibility of our security
   process. See the
